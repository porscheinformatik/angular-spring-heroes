...
export class HeroService {
  ...
  update(hero: Hero) {
    return this.http.put<Hero>(`api/heroes/${hero.id}`, hero)
      .pipe(tap(changedHero => {
        const currentList = this.heroes$.value;
        const changedIdx = currentList.findIndex(h => h.id === changedHero.id);
        this.updateList([...currentList.slice(0, changedIdx), changedHero, ...currentList.slice(changedIdx+1)]);
      }));
  }
  ...
  delete(hero: Hero) {
    return this.http.delete<Hero>(`api/heroes/${hero.id}`).pipe(tap(_ => {
